...
echo "chmod +x shadowsocks-libev.sh"
echo "./shadowsocks-libev.sh 2>&1 | tee shadowsocks-libev.log"
echo "#############################################################"
echo ""
...
function iptables_set(){
    ...
    if [ $? -eq 0 ]; then
        /etc/init.d/iptables status | grep '8080' | grep 'ACCEPT' >/dev/null 2>&1
        if [ $? -ne 0 ]; then
...
function install(){
    ...
    echo -e "Your Server Port: \033[41;37m 8080 \033[0m"
